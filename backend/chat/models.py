...
class ChatHistory(models.Model):
    """
    Stocke l'état de la mémoire 'ConversationSummaryBufferMemory' pour un utilisateur.
    """
    user = models.OneToOneField(
        settings.AUTH_USER_MODEL, 
        on_delete=models.CASCADE, 
    ...
    # MODIFICATION 1 : Pour le RÉSUMÉ (la mémoire long terme)
    summary = models.TextField(
        blank=True, 
        null=True,
        help_text="Résumé intelligent de la conversation passée."
    )
    
    # MODIFICATION 2 : Pour les MESSAGES RÉCENTS (la mémoire court terme / tampon)
    recent_messages = models.JSONField(
        default=list, 
        blank=True,
        help_text="Les X derniers messages bruts (format LangChain JSON)."
    )

    last_updated = models.DateTimeField(auto_now=True)
    
    def __str__(self):
        return f"Mémoire de conversation pour {self.user.email}"
